...
import thelost.powers.SacredOrbPower;
import thelost.util.CardInfo;

import static thelost.BasicMod.makeID;

public class SacredOrb extends BaseCard {
    private final static CardInfo cardInfo = new CardInfo(
            "SacredOrb", //Card ID. Will be prefixed with mod id, so the final ID will be "modID:MyCard" with whatever your mod's ID is.
    ...
    public void use(AbstractPlayer p, AbstractMonster m) {
        ...
        addToBot(new ApplyPowerAction(p, p, new SacredOrbPower(p, -1), -1));
    }

    @Override
    public AbstractCard makeCopy() {
        return new SacredOrb();
    }
}
