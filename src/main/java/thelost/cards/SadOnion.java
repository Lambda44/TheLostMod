...
import thelost.powers.SadOnionPower;
import thelost.powers.TearsPower;
import thelost.util.CardInfo;

import static thelost.BasicMod.makeID;

public class SadOnion extends BaseCard {
    private final static CardInfo cardInfo = new CardInfo(
            "SadOnion", //Card ID. Will be prefixed with mod id, so the final ID will be "modID:MyCard" with whatever your mod's ID is.
    ...
    public SadOnion() {
        ...
        setMagic(TEARS, TEARS_UPG);
    }

    @Override
    public void use(AbstractPlayer p, AbstractMonster m) {
        addToBot(new ApplyPowerAction(p, p, new SadOnionPower(p, this.magicNumber), this.magicNumber));
    }
