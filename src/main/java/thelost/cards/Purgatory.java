...
import thelost.powers.PurgatoryPower;
import thelost.util.CardInfo;

import static thelost.BasicMod.makeID;

public class Purgatory extends BaseCard {
    private final static CardInfo cardInfo = new CardInfo(
            "Purgatory", //Card ID. Will be prefixed with mod id, so the final ID will be "modID:MyCard" with whatever your mod's ID is.
    ...
    private static final int GHOSTS = 2;
    private static final int UPG_GHOSTS = 2;

    public Purgatory() {
        super(cardInfo); //Pass the cardInfo to the BaseCard constructor. use true as a 2nd argument to change description

        setMagic(GHOSTS, UPG_GHOSTS);
    }

    @Override
    public void use(AbstractPlayer p, AbstractMonster m) {
        this.addToBot(new ApplyPowerAction(p, p, new PurgatoryPower(p, this.magicNumber), this.magicNumber));
    }

    @Override
    public AbstractCard makeCopy() {
        return new Purgatory();
    }
}
