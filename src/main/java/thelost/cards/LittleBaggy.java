...
public class LittleBaggy extends BaseCard {
    private final static CardInfo cardInfo = new CardInfo(
            "LittleBaggy", //Card ID. Will be prefixed with mod id, so the final ID will be "modID:MyCard" with whatever your mod's ID is.
    ...
    private static final int CRY = 6;
    private static final int UPG_CRY = 3;
    ...
    public LittleBaggy() {
        ...
        setBlock(CRY, UPG_CRY);
        setMagic(EVIL);
    }

    @Override
    public void use(AbstractPlayer p, AbstractMonster m) {
        addToBot(new ApplyPowerAction(p, p, new TearsPower(p, block), block));
