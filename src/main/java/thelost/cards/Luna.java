...
import thelost.powers.LunaPower;
import thelost.powers.ToothPower;
...
public class Luna extends BaseCard {
    private final static CardInfo cardInfo = new CardInfo(
            "Luna", //Card ID. Will be prefixed with mod id, so the final ID will be "modID:MyCard" with whatever your mod's ID is.
    ...
    private static final int PERFECT_UPG = 1;

    public Luna() {
        super(cardInfo); //Pass the cardInfo to the BaseCard constructor. use true as a 2nd argument to change description

        setMagic(PERFECT, PERFECT_UPG);
    }

    @Override
    public void use(AbstractPlayer p, AbstractMonster m) {
        this.addToBot(new ApplyPowerAction(p, p, new LunaPower(p, this.magicNumber), this.magicNumber));
    }
