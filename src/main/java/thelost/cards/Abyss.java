...
public class Abyss extends BaseCard {
    private final static CardInfo cardInfo = new CardInfo(
            "Abyss", //Card ID. Will be prefixed with mod id, so the final ID will be "modID:MyCard" with whatever your mod's ID is.
    ...
    private static final int RAD_UPG = 1;
    ...
    public void use(AbstractPlayer p, AbstractMonster m) {
        for(int i = 0; i < this.magicNumber; ++i) {
            this.addToTop(new DamageAllEnemiesAction(p, damage, this.damageTypeForTurn, AbstractGameAction.AttackEffect.FIRE));
        }
    }
